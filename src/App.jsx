...
function App() {
  ...
  const [apps] = useAppData(); // <-- Corrección aquí (no uses setApps si no lo necesitas)
  ...
  return (
    <>
          ...
          {opciones.map((opcion, i) => (
            ...
            >
              <div className="logos">
                ...
                />
                <span className="nombre-logo">{opcion.tipo}</span>
              </div>
            </EngineButton>
          ))}
        </div>
      ...
      {/* Apps conectadas dinámicamente */}
     <div className="absolute top-2 left-0 right-0 flex justify-center gap-8 px-4 py-4 text-sm text-gray-600">
  {apps.map((app, index) => (
    <a
      key={index}
      href={app.url}
